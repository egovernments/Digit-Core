package org.egov.infra.persist.auditclient;
...
@Service
@Slf4j
public class PersisterConfigParser {
    ...
    public void generateAuditLogs(String topic, String json) {
    ...
    private Map<AuditAttributes, List<LinkedHashMap<String, Object>>> extractData(String baseJsonPath, Mapping mapping, Object document) {
        ...
        try {
            if (baseJsonPath.contains("*")) {
                ...
                if (relativeJsonPath.contains(".*"))
                    relativeJsonPath = relativeJsonPath.substring(relativeJsonPath.lastIndexOf(".*") + 2);
                ...
                for (int i = 0; i < parentObjects.size(); i++) {
                    AuditAttributes auditAttributes = getAuditAttribute(mapping, parentObjects.get(i));
                    if (relativeJsonPath.contains(".*"))
                        list = JsonPath.read(parentObjects.get(i), relativeJsonPath);
                    else list = Collections.singletonList(parentObjects.get(i));
