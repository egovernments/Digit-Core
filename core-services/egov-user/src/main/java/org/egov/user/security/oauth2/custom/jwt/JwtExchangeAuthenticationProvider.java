...
public class JwtExchangeAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) {
        ...
        // NOTE: SSO failures are deliberately NOT wrapped in OAuth2AuthenticationException here.
        // OAuth2AuthenticationException extends AuthenticationException, and
        // CustomAuthenticationManager.authenticate() catches AuthenticationException, logs it and
        // continues the provider loop — so a wrapped SSO failure is swallowed and the caller gets the
        // generic "AUTHENTICATION_FAILURE" 400 instead of the SSO error code. SsoException extends
        // RuntimeException, so letting it propagate reaches CustomWebResponseExceptionTranslator and
        // surfaces the real error code and status.
    ...
    private JwtExchangeInput extractJwtExchangeInput(Authentication authentication) {
        ...
        if (token == null || token.trim().isEmpty()) {
            throw new IdpJwtValidationException(SsoErrorCodes.JWT_INVALID, "Token cannot be null or empty");
        }

        // Validate token size to prevent DoS attacks (max 10KB)
        if (token.length() > 10240) {
            throw new IdpJwtValidationException(SsoErrorCodes.JWT_INVALID, "Token size exceeds maximum allowed limit");
        }
    ...
    private void validateTokenReplay(OidcValidatedJwt jwt, String tenantId) {
        ...
        if (tokenId == null || tokenId.isEmpty()) {
            throw new IdpJwtValidationException(SsoErrorCodes.JWT_INVALID,
                    JwtConstants.ERROR_MISSING_TOKEN_ID);
        }
    ...
    private User unlockAccount(User user, RequestInfo requestInfo) {
        User userToBeUpdated = user.toBuilder()
                ...
                .password(null)
                .build();
        // SSO path: no password in the JWT-derived User, so preserve the stored hash.
        return userService.updateWithoutOtpValidation(userToBeUpdated, requestInfo, true);
    }
