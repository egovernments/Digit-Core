...
## 1.2.9 - 2026-06-16
- Added international mobile number validation driven by MDMS-v2 (`common-masters.UserValidation` schema).
...
- Validation config cached in Redis under hash key `validationRules`, field `validation:{stateTenantId}:{countryCode}`; TTL configurable via `egov.validation.cache.ttl.seconds` (default 3600 s).
- Falls back to the MDMS default entry when no country code is supplied or no matching entry exists; throws `VALIDATION_CONFIG_MISSING` only when no config is found at all.
...
- **Backward compatible**: `countrycode` column is nullable; existing rows default to `+91` behaviour at runtime without data migration.
