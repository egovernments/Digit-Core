...
public class RbacFilter implements GlobalFilter , Ordered {
    ...
    public RbacFilter(ModifyRequestBodyGatewayFilterFactory modifyRequestBodyFilter, RbacFilterHelper rbacFilterHelper, ObjectMapper objectMapper, MultiStateInstanceUtil centralInstanceUtil, CommonUtils commonUtils, RestTemplate restTemplate, ApplicationProperties applicationProperties) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(Boolean.TRUE.equals(rbacFlag)) {
            ...
            if(contentType == null || (contentType.contains(FORM_DATA_TYPE) || contentType.contains(X_WWW_FORM_URLENCODED_TYPE))){
                return helpRBAC(exchange,chain);
            }
            else{
    ...
    public Mono<Void> helpRBAC(ServerWebExchange exchange, GatewayFilterChain chain) {

        isIncomingURIInAuthorizedActionList(exchange,chain);
        return chain.filter(exchange);
    }

    private void isIncomingURIInAuthorizedActionList(ServerWebExchange exchange, GatewayFilterChain chain) {

        String requestUri = exchange.getRequest().getURI().getPath();
        System.out.println(MDC.get(USER_INFO_KEY));

        User user = null;
        try {
            user = objectMapper.readValue(MDC.get(USER_INFO_KEY), User.class);
        ...
        List<String> tenantIdList = exchange.getRequest().getHeaders().get("tenantId");
        Set<String> tenantIds=Collections.emptySet();
        if(!CollectionUtils.isEmpty(tenantIdList)){
           tenantIds = new HashSet<>(tenantIdList);
        }
        else{
            throw new CustomException(TENANT_ID_KEY, "TenantId not found in header");
