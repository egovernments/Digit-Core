...
import static org.egov.handler.config.ServiceConstants.TENANT_CITIES_SCHEMA;
...
public class DataHandlerService {
	...
	private void createTenantBoundarydata(RequestInfo requestInfo, String targetTenantId) {
		...
		List<String> codeList = getBoundaryCodes(requestInfo, targetTenantId);
		...
		for (String code : codeList) {
			...
			for (Mdms mdms : mdmsList) {
				...
				if (dataNode.has("boundary") && dataNode.has("hierarchyType")) {
					ObjectNode boundaryNode = (ObjectNode) dataNode.get("boundary");
					boundaryNode.put("code", code);

					String hierarchyTypeCode = dataNode.get("hierarchyType").get("code").asText();
					String newUniqueIdentifier = hierarchyTypeCode + "." + code;

					mdms.setUniqueIdentifier(newUniqueIdentifier);
					((ObjectNode) dataNode).put("boundaryName", newUniqueIdentifier);

					((ObjectNode) dataNode).set("boundary", boundaryNode);
					mdms.setData(dataNode);
				} else {
					log.info("Boundary (or) Hierarchy type node does not exist in the data.");
				}
	...
	private List<String> getBoundaryCodes(RequestInfo requestInfo, String targetTenantId) {
		List<String> boundaryCodeList = new ArrayList<>();
		boundaryCodeList.add(targetTenantId);

		List<Mdms> mdmsList = getAllMdmsResults(serviceConfig.getDefaultTenantId(), TENANT_CITIES_SCHEMA, requestInfo);

		for (Mdms mdms : mdmsList) {
			if (mdms.getData() != null) {
				String code = mdms.getData().get("code").asText();
				boundaryCodeList.add(code);
				boundaryCodeList.add(targetTenantId + "." + code);
			}
		}
