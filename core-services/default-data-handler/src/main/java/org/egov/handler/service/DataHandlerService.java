...
public class DataHandlerService {
	...
	public void importKeycloakRealm(TenantRequest tenantRequest) {
		...
		Keycloak keycloak = KeycloakBuilder.builder()
				...
				.realm(serviceConfig.getKeycloakRealm())
				.clientId(serviceConfig.getKeycloakClientId())
				.username(serviceConfig.getKeycloakAdminUsername())
				.password(serviceConfig.getKeycloakAdminPassword())
				.build();
	...
	private RealmRepresentation loadAndCreateRealmConfig(TenantRequest tenantRequest) {
		...
		Map<String, String> placeholders = Map.of(
				"{{tenantCode}}", tenantRequest.getTenant().getCode(),
				"{{tenantCodeLowerCase}}", tenantRequest.getTenant().getCode().toLowerCase(),
				"{{tenantEmail}}", tenantRequest.getTenant().getEmail(),
				"{{tenantName}}", tenantRequest.getTenant().getName(),
				"{{mobileNumber}}", "9999999999"
		);

		// Load source realm JSON
		Resource resource = resourceLoader.getResource("classpath:SourceRealmConfig.json");
		try (InputStream inputStream = resource.getInputStream()) {
			...
			return objectMapper.readValue(json, RealmRepresentation.class);
		} catch (IOException e) {
